...
[![PyPI](https://img.shields.io/pypi/v/tenx-cli.svg)](https://pypi.org/project/tenx-cli/)
[![PyPI Downloads](https://img.shields.io/pypi/dm/tenx-cli.svg)](https://pypistats.org/packages/tenx-cli)
...
```text
               ┌────────────────────────────────────────────────────────┐
               │              HUMAN / LEAD AGENT PROMPT                 │
               └──────────────────────────┬─────────────────────────────┘
                                          │
                                          ▼
                      ┌───────────────────────────────────────┐
                      │    tenx CONTEXT BASE (.tenx/)         │
                      │ ┌──────────┐ ┌─────────┐ ┌──────────┐ │
                      │ │  Epics   │ │  Specs  │ │Conventions││
                      │ └────┬─────┘ └────┬────┘ └────┬─────┘ │
                      └──────┼────────────┼───────────┼───────┘
                             │            │           │
           ┌─────────────────┴────────────┼───────────┴─────────────────┐
           ...
           ▼                              ▼                             ▼
┌───────────────────────┐      ┌─────────────────────┐      ┌───────────────────────┐
│     TOPOLOGICAL       │      │  AUTONOMOUS SUBAGENT│      │     SDLC LINTING      │
│     DAG SWARM         │      │  ISOLATED WORKTREE  │      │     & GATES           │
│                       │      │                     │      │                       │
│  tenx dag / swarm     │─────▶│  tenx dispatch      │─────▶│  tenx validate        │
│  Parallel Wave        │      │  Exact commit clone │      │  Evidence verification│
│  Execution            │      │  Visual Herdr / Tmux│      │  Pre-commit git hook  │
└───────────────────────┘      └─────────────────────┘      └───────────────────────┘
                                          │
                                          ▼
                               ┌─────────────────────┐
...
| Capability | Ad-Hoc Prompting / Rules Files | Heavy Agent Frameworks | **tenx Meta-Harness** |
| :--- | :---: | :---: | :---: |
| **Runtime Dependencies** | 0 (plain text) | Heavy (LangChain, CrewAI, etc.) | **0 runtime deps (Python stdlib)** |
| **Context Longevity** | Degrades over long chats | Stored in black-box vector DBs | **Git-native markdown artifacts** |
| **Execution Safety** | None (silent drift) | Runtime sandboxes | **Git pre-commit gate + SDLC linter** |
| **Multi-Agent Orchestration** | Manual copy-pasting | Opaque message bus | **Topological DAG Swarm + Visual mux** |
| **Universal Harness Support** | Configured per IDE | Vendor locked | **29+ harnesses (Claude, Cursor, Codex)** |
| **Tool Protocol** | None | Proprietary APIs | **19 built-in standard MCP tools** |

## Install

```bash
# Install directly via uv (recommended)
uv tool install tenx-cli

# Or via pipx
pipx install tenx-cli

# Or via pip
pip install tenx-cli

# Verify install (executable command is 'tenx')
tenx --version
```
