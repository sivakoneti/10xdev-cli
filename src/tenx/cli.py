...
Command map (autonomous meta-harness architecture):
...
def cmd_reconcile(args: argparse.Namespace) -> int:
    ...
    res = reconcile_subagent_ticket(project_root=root, ticket_id=ticket_id, skip_verify=skip_verify)
    ...
    if res.status == "merged":
        ...
        if res.spec_id:
            print(f"  spec:       {res.spec_id} (ticket marked done)")
        print(f"  branch:     {res.branch} (deleted)")
    ...
    if res.status == "verified_failed":
        print(f"Pre-landing verification failed for {res.ticket_id}:", file=sys.stderr)
        print(res.verification_output or "", file=sys.stderr)
        print(f"\nMerge aborted. Worktree preserved for inspection at: {res.worktree_path}", file=sys.stderr)
        return 1

    if res.status == "conflict":
        print(f"Merge conflict encountered while merging {res.branch}:", file=sys.stderr)
        print(res.error or "", file=sys.stderr)
        print(f"\nWorktree preserved for inspection at: {res.worktree_path}", file=sys.stderr)
        return 1

    print(f"Reconciliation error for {res.ticket_id}: {res.error}", file=sys.stderr)
    return 1


def cmd_abort(args: argparse.Namespace) -> int:
    """Abort and discard a subagent ticket worktree and multiplexer session."""
    from .reconcile import abort_subagent_ticket
    root = _root_or_die(args.root)
    _require_init(root)
    ticket_id = args.ticket
    res = abort_subagent_ticket(project_root=root, ticket_id=ticket_id)
...
def cmd_dag(args: argparse.Namespace) -> int:
    """Visualize and inspect DAG ticket dependencies and topological waves."""
    root = _root_or_die(args.root)
    _require_init(root)
    from .dag import build_spec_dag, CyclicDependencyError
    harness = load_harness(root)
    spec = harness.get(args.spec.upper())
    if spec is None or spec.type != "spec":
        print(f"tenx: unknown spec {args.spec}", file=sys.stderr)
    ...
    try:
        ...
        print(f"tenx: cyclic dependency detected in {spec.id}: {e}", file=sys.stderr)
        return 1
    except Exception as e:
        print(f"tenx: failed to build DAG for {spec.id}: {e}", file=sys.stderr)
        return 1
    ...
    print(f"# DAG execution waves for {dag.spec_id} ({len(dag.waves)} wave(s))")
    for idx, wave in enumerate(dag.waves, 1):
        print(f"\nWave {idx}:")
        for tid in wave:
            node = dag.nodes[tid]
            dep_str = f" (depends on: {', '.join(node.depends_on)})" if node.depends_on else ""
            print(f"  - [{node.status}] {node.id}: {node.title}{dep_str}")
    return 0


def cmd_swarm(args: argparse.Namespace) -> int:
    """Schedule and coordinate wave-based parallel ticket execution."""
    root = _root_or_die(args.root)
    _require_init(root)
    from .swarm import execute_spec_swarm, plan_spec_swarm
    harness = load_harness(root)
    spec = harness.get(args.spec.upper())
    if spec is None or spec.type != "spec":
        print(f"tenx: unknown spec {args.spec}", file=sys.stderr)
    ...
    if args.dry_run:
        ...
        if getattr(args, "json", False):
            print(json.dumps({
                ...
                "status": "planned",
                "waves": plan.waves,
                "ready_tickets": plan.ready_tickets,
                "blocked_tickets": plan.blocked_tickets,
                "completed_tickets": plan.completed_tickets,
            }, indent=2))
            return 0
        print(f"# Swarm plan for {plan.spec_id} (Dry Run)")
        print(f"Total tickets: {plan.total_tickets} (Completed: {len(plan.completed_tickets)}, Ready: {len(plan.ready_tickets)}, Blocked: {len(plan.blocked_tickets)})")
        for idx, wave in enumerate(plan.waves, 1):
            print(f"  Wave {idx}: {', '.join(wave)}")
        print(f"\nReady to dispatch: {', '.join(plan.ready_tickets) if plan.ready_tickets else 'none'}")
        return 0
    ...
    if getattr(args, "json", False):
        print(json.dumps({
            "spec_id": res.spec_id,
            "status": res.status,
            "executed_waves": res.executed_waves,
            "dispatched_tickets": res.dispatched_tickets,
            "reconciled_tickets": res.reconciled_tickets,
            "failed_tickets": res.failed_tickets,
            "details": res.details,
        }, indent=2))
        return 0 if res.status == "completed" else 1

    print(f"# Swarm execution for {res.spec_id}: {res.status}")
    print(f"Details: {res.details}")
    if res.dispatched_tickets:
        print(f"Dispatched: {', '.join(res.dispatched_tickets)}")
    if res.reconciled_tickets:
        print(f"Reconciled: {', '.join(res.reconciled_tickets)}")
    if res.failed_tickets:
        print(f"Failed: {', '.join(res.failed_tickets)}")
    return 0 if res.status == "completed" else 1
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for cmd_name in ("reconcile", "merge"):
        sp = sub.add_parser(cmd_name,
                            help="verify, merge, and teardown a subagent ticket worktree")
        sp.add_argument("ticket", help="ticket ID, e.g. SPC-031-T2")
        sp.add_argument("--skip-verify", action="store_true",
                        help="skip pre-landing validation checks (not recommended)")
        sp.add_argument("--json", action="store_true")
        sp.set_defaults(func=cmd_reconcile)

    sp = sub.add_parser("abort",
                        help="abort and cleanly discard a subagent ticket worktree and multiplexer session")
    sp.add_argument("ticket", help="ticket ID, e.g. SPC-031-T2")
    sp.add_argument("--json", action="store_true")
    sp.set_defaults(func=cmd_abort)

    sp = sub.add_parser("dag",
                        help="visualize and inspect ticket dependencies and execution waves")
    sp.add_argument("spec", help="spec ID, e.g. SPC-033")
    sp.add_argument("--json", action="store_true")
    sp.set_defaults(func=cmd_dag)
    ...
    sp.add_argument("--dry-run", action="store_true",
                    help="plan and show wave execution without launching agents")
    sp.add_argument("--json", action="store_true")
