...
2. **Dispatch Worker:**
   - **CLI Headless Mode (Background):**
     ...
     worker headlessly in the background.
   - **Visual Multiplexer Mode (Herdr Sidenav / Tmux Window):**
     Run `tenx dispatch <SPEC-ID> <TICKET-ID> --agent <name> --visual`.
     When running inside **Herdr** (`HERDR_ENV=1` or active server), this creates
     an isolated Herdr workspace labeled with the ticket ID (`--no-focus` by default).
     The subagent appears immediately in **Herdr's left sidebar navigation** (sidenav),
     allowing the operator to visually monitor terminal activity in real-time or switch to it.
     When running inside **tmux**, this creates a background tmux window named after the ticket.
