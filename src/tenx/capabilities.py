...
CAPABILITIES: list[dict[str, str]] = [
    ...
    {"name": "reconcile", "surface": "both", "group": "track",
     ...
     "when": "After a subagent finishes implementing and validating its ticket."},
    {"name": "merge", "surface": "cli", "group": "track",
     "usage": "tenx merge <TICKET-ID> [--skip-verify] [--json]",
     "what": "Alias for `tenx reconcile`.",
     "when": "Landing completed subagent work into main."},
    {"name": "abort", "surface": "both", "group": "track",
     "usage": "tenx abort <TICKET-ID> [--json]",
     "what": "Safely discard a failed or abandoned subagent worktree and workspace with zero residual state.",
     "when": "Discarding an aborted or failed subagent ticket branch."},
    {"name": "dag", "surface": "both", "group": "plan",
     "usage": "tenx dag <SPEC-ID> [--json]",
