...
### Fixed
- Fixed the updater to always check the tenx distribution repository instead of the caller's Git remote.
...
### Added
- feat(multiplexer): format projected Herdr workspace labels with branching glyphs and integrate native agent start lifecycle
- Implement subagent reconciliation engine: verification, merge, and multiplexer teardown (tenx reconcile, tenx abort) (SPC-032)
- Swarm and DAG subagent dispatch with wave partitioning, parallel execution, interactive visual chat, and MCP tools (SPC-033)
- Implement reactive subagent wait and event-driven swarm synchronization (SPC-034)

### Changed
- docs(readme): update README with subagent dispatch, visual projection, Bifrost routing, and memory distillation
- chore(sanitize): scrub personal author names and references for open source release
- docs(oss): prepare repository documentation, sanitize personal metadata, and document v0.24.0 dispatch (SPC-031)
